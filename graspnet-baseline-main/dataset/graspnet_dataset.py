...
class GraspNetDataset(Dataset):
    def __init__(self, root, valid_obj_idxs, grasp_labels, camera='kinect', split='train', num_points=20000,
        ...
        # Giới hạn số scene để giảm RAM khi load collision labels
        if split == 'train':
            keep_n = int(len(self.sceneIds) * 0.50)   # 100 -> 50 scene
            self.sceneIds = self.sceneIds[:keep_n]
        # Bỏ giới hạn đối với test vì test không load collision_labels (load_label=False)
    ...
    def get_data_label(self, index):
        ...
        for i, obj_idx in enumerate(obj_idxs):
            ...
            max_grasp_per_obj = 100
            idxs = np.random.choice(len(points), min(max_grasp_per_obj, len(points)), replace=False)    
