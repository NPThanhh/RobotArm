...
def process_grasp_labels(end_points):
    ...
    for i in range(len(clouds)):
        ...
        if len(grasp_points_merged) == 0:
            print("EMPTY grasp_points_merged")
            print("batch index:", i)
            print("poses len:", len(poses))
            print("object_poses_list:", end_points['object_poses_list'][i])
            print("grasp_points_list len:", len(end_points['grasp_points_list'][i]))
            raise RuntimeError("No valid grasp labels for this sample")
