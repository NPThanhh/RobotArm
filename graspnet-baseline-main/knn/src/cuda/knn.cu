...
/**
  ...
  */
__global__ void cuInsertionSort(float *dist, int64_t *ind, int64_t width, int64_t height, int64_t k){
...
void knn_device(
  ...
  cuComputeDistanceGlobal<<<g_16x16, t_16x16, 0, stream>>>(
    ref_dev,
    ref_width,
    query_dev,
    query_width,
  ...
  // cuParallelSqrt<<<g_k_16x16, t_k_16x16, 0, stream>>>(dist_dev, query_width, k);

#if DEBUG
  unsigned int size_of_float = sizeof(float);
  size_t size_of_int64 = sizeof(int64_t);

  float* dist_host = new float[query_width * k];
  int64_t* idx_host = new int64_t[query_width * k];

  // Memory copy of output from device to host
  cudaMemcpy(
    &dist_host[0],
    dist_dev,
    query_width * k * size_of_float,
